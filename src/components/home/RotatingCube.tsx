...
export default function RotatingCube({ brands, visibleFaceIndex, onFaceClick, isRotating }: RotatingCubeProps) {
  ...
  const mapIndexToQuaternion = (index: number): THREE.Quaternion => {
    switch (index) {
        case 0: return new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0, 0));
        case 1: return new THREE.Quaternion().setFromEuler(new THREE.Euler(0, -Math.PI / 2, 0));
        case 2: return new THREE.Quaternion().setFromEuler(new THREE.Euler(0, -Math.PI, 0));
        case 3: return new THREE.Quaternion().setFromEuler(new THREE.Euler(0, Math.PI / 2, 0));
        case 4: return new THREE.Quaternion().setFromEuler(new THREE.Euler(Math.PI / 2, 0, 0));
        case 5: return new THREE.Quaternion().setFromEuler(new THREE.Euler(-Math.PI / 2, 0, 0));
