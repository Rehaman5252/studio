...
export default function CertificatesContent() {
  ...
  const certificates = useMemo(() => {
    return quizHistory
      ...
      .map(attempt => ({
        ...
        userName: user?.displayName || 'Valued Player'
      }));
  }, [quizHistory, user]);
  ...
  const handleDownload = (cert: typeof certificates[0]) => {
    ...
    doc.text(cert.userName, doc.internal.pageSize.width / 2, 70, { align: 'center' });
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0);
  ...
  if (isLoading || authLoading) {
    return (
        <div className="space-y-4">
            <CertificateItemSkeleton /><CertificateItemSkeleton />
        </div>
    );
  }
