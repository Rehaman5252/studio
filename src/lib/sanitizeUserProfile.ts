...
export function sanitizeQuizAttempt(raw: any): Partial<QuizAttempt> {
  if (!raw || typeof raw !== 'object') {
    // Return a minimal structure if input is completely invalid
    return {
      userId: "",
      slotId: "",
      brand: "unknown",
  ...
  // Pad answers array to match the number of questions
  while (answers.length < sanitized.totalQuestions) {
    answers.push(""); // Pad with empty string for unanswered
  }
  sanitized.userAnswers = answers.slice(0, sanitized.totalQuestions); // Ensure it's not longer
  ...
  // Pad timePerQuestion array to match the number of questions
